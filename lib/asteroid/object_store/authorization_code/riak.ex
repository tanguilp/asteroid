defmodule Asteroid.ObjectStore.AuthorizationCode.Riak do
  @moduledoc """
  Riak implementation of the `Asteroid.ObjectStore.AuthorizationCode` behaviour
  ...
  - it installs a custom schema (`asteroid_object_store_authorization_code_riak_schema`)
  - it creates a new index (`asteroid_object_store_authorization_code_riak_index`) on the bucket
  ...
  def install(opts) do
    ...
    with :ok <-
           Riak.Search.Schema.create(
             schema_name(),
             (:code.priv_dir(:asteroid) ++ '/riak/object_store_authorization_code_schema.xml')
             |> File.read!()
           ),
  ...
  def put(authorization_code, opts) do
    ...
    riak_map =
      if authorization_code.data["exp"] != nil do
        ...
        Logger.warn(
          "Inserting authorization code with no expiration: #{
            String.slice(authorization_code.id, 1..5)
          }..."
        )
  ...
  @spec search(String.t(), Asteroid.ObjectStore.AuthorizationCode.opts()) ::
  ...
  defp schema_name(), do: "asteroid_object_store_authorization_code_riak_schema"

  @doc false

  @spec index_name() :: String.t()

  def index_name(), do: "asteroid_object_store_authorization_code_riak_index"
end
