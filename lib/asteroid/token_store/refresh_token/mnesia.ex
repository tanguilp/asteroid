defmodule Asteroid.TokenStore.RefreshToken.Mnesia do
  @moduledoc """
  Mnesia implementation of the `Asteroid.TokenStore.RefreshToken` behaviour
  ...
  ```elixir
  [
    attributes: [:id, :subject_id, :client_id, :device_id, :data],
    disc_copies: [node()]
  ]
  ```
  ...
  def get(refresh_token_id, opts) do
    ...
    case :mnesia.dirty_read(table_name, refresh_token_id) do
      ...
      [{^table_name, ^refresh_token_id, _subject_id, _client_id, _device_id, data}] ->
        refresh_token =
          RefreshToken.new(
            id: refresh_token_id,
            data: data
  ...
  def get_from_subject_id(subject_id, opts) do
    ...
    {:ok,
      for {_table_name, refresh_token_id, _subject_id, _client_id, _device_id, _data} <-
        :mnesia.dirty_match_object({table_name, :_, subject_id, :_, :_, :_}) do
        refresh_token_id
      end
    }
  catch
  ...
  def get_from_client_id(client_id, opts) do
    ...
    {:ok,
      for {_table_name, refresh_token_id, _subject_id, _client_id, _device_id, _data} <-
        :mnesia.dirty_match_object({table_name, :_, :_, client_id, :_, :_}) do
        refresh_token_id
      end
    }
  catch
  ...
  def get_from_device_id(device_id, opts) do
    ...
    {:ok,
      for {_table_name, refresh_token_id, _subject_id, _client_id, _device_id, _data} <-
        :mnesia.dirty_match_object({table_name, :_, :_, :_, device_id, :_}) do
        refresh_token_id
      end
    }
  catch
