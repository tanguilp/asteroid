defmodule Asteroid.Store.AccessToken.Riak do
  @moduledoc """
  Riak implementation of the `Asteroid.Store.AccessToken` behaviour
  ...
  ## Options
  The options (`Asteroid.Store.AccessToken.opts()`) are:
  ...
  - it installs a custom schema (`asteroid_token_store_access_token_riak_schema`)
  - it creates a new index (`asteroid_token_store_access_token_riak_index`) on the bucket
  ...
  def install(opts) do
    ...
    with :ok <-
           Riak.Search.Schema.create(
             schema_name(),
             (:code.priv_dir(:asteroid) ++ '/riak/token_store_access_token_schema.xml')
             |> File.read!()
           ),
  ...
  ```elixir
  iex(13)> Asteroid.Store.AccessToken.Riak.search("sub_register:j* AND exp_int_register:[0 TO #{:os.system_time(:seconds)}]", opts)
  {:ok, ["7WRQL4EAKW27C5BEFF3JDGXBTA", "WCJBCL7SC2THS7TSRXB2KZH7OQ"]}
  ```
  """

  @spec search(String.t(), Asteroid.Store.AccessToken.opts()) ::
  ...
  defp schema_name(), do: "asteroid_token_store_access_token_riak_schema"

  @doc false

  @spec index_name() :: String.t()

  def index_name(), do: "asteroid_token_store_access_token_riak_index"
end
