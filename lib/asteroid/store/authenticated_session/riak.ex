defmodule Asteroid.Store.AuthenticatedSession.Riak do
  @moduledoc """
  Riak implementation of the `Asteroid.Store.AuthenticatedSession` behaviour
  ...
  - it installs a custom schema (`asteroid_token_store_authenticated_session_riak_schema`)
  - it creates a new index (`asteroid_token_store_authenticated_session_riak_index`) on the bucket
  ...
  def install(opts) do
    ...
    with :ok <-
           Riak.Search.Schema.create(
             schema_name(),
             (:code.priv_dir(:asteroid) ++ '/riak/token_store_authenticated_session_schema.xml')
             |> File.read!()
           ),
  ...
  @spec search(String.t(), Asteroid.Store.AuthenticatedSession.opts()) ::
  ...
  defp schema_name(), do: "asteroid_token_store_authenticated_session_riak_schema"

  @doc false

  @spec index_name() :: String.t()

  def index_name(), do: "asteroid_token_store_authenticated_session_riak_index"
end
